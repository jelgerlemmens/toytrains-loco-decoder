...
void ExecuteProgrammedState(){
    //running on an anlog track, coming from digital, switch state to analog and reset
    if(AnalogRunCheck::CheckIfAnalog()){
      #ifdef DEBUG  
        SERIALINTERFACE.print("Switching from Digital to Analog track ops"); 
      #endif        
...
void ExecuteAnalogState(){
    //no dcc init, just the input pin to evaluate the state of the track
    pinMode(DCC_INPUT_PIN, INPUT);
    // for a 1/4 of a sec check if state is still analog 
    if(!AnalogRunCheck::CheckIfAnalog()){
      #ifdef DEBUG  
        SERIALINTERFACE.print("Switching from Analog to Digital track ops");
      #endif
...
void notifyDccSpeed(uint16_t Addr, DCC_ADDR_TYPE AddrType, uint8_t Speed, DCC_DIRECTION Dir, DCC_SPEED_STEPS SpeedSteps)
{
  ...
  if (Speed > 1)
  {
    //map speed to PWM signal range note that the TLE5206 uses an inverted range
    Speed = map(Speed,2,126,255,25);
      #ifdef DEBUG      
        SERIALINTERFACE.print("Direction: "); //rev = 0 . fwd = 1
        SERIALINTERFACE.println(Dir, DEC); 
        SERIALINTERFACE.print("Speed: ");      
        SERIALINTERFACE.println(Speed, DEC); 
      #endif

    //going forward, set hbridge accordingly 
    if (Dir == DCC_DIR_FWD)
    { 
    ...
    //reverse, set hbridge accordingly 
    if (Dir == DCC_DIR_REV)
    {
      //Set tle5206 to reverse
      digitalWrite(MOTOR_OUT_DIR_FWD_PIN, HIGH);      
      analogWrite(MOTOR_OUT_DIR_REV_PIN, Speed);      
    }
  }
}
//-----------------------------------------------------------------------------------------------------------------------------
